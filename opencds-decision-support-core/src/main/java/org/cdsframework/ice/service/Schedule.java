...
public class Schedule
{
            ...
            throws IllegalArgumentException, InconsistentConfigurationException
    {
        ...
        if (pScheduleId == null || pCommonLogicModule == null || ObjectUtils.isEmpty(pKnowledgeModules))
        {
    ...
    /**
     * Returns true if this schedule supports the specified knowledge module
     * <p>
     * Can be used in an eval condition to make a rule fire only if the schedule supports the specified knowledge module:
     * example: eval(schedule.isKnowledgeModule("org.cdsframework^ICE^1.0.0"))
     */
    public boolean isKnowledgeModule(final String kmId)
    {
        return getICESupportingDataConfiguration().getSupportedCdsLists().getCdsVersions().contains(kmId);
    }

    public List<String> getKnowledgeModules()
    {
        return getICESupportingDataConfiguration().getSupportedCdsLists().getCdsVersions();
    }
