...
public class IceExecutionEngineContext implements ExecutionEngineContext<List<Command<?>>, ExecutionResults>
{
    ...
    public List<Command<?>> getInput()
    {
        ...
        final Date evalTime = evaluationContext.getEvalTime();
         ...
         */
        for (final Map.Entry<String, Object> global : evaluationContext.getGlobals().entrySet())
        {
        ...
        if (!ObjectUtils.isEmpty(evaluationContext.getPrimaryProcess()))
        {
            cmds.add(CommandFactory.newStartProcess(evaluationContext.getPrimaryProcess()));
            if (log.isDebugEnabled())
                log.debug("knowledgeBase Primary Process: {}", evaluationContext.getPrimaryProcess());
        }
    ...
    public ExecutionEngineContext<List<Command<?>>, ExecutionResults> setResults(final ExecutionResults results)
    {
        ...
        // from drools.
        resultFactLists.put(ASSERTIONS, List.of(evaluationContext.getNamedObjects()));
        resultFactLists.put(NAMED_OBJECTS, List.of(evaluationContext.getNamedObjects()));
