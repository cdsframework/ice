...
public class VaccineComponent extends AbstractVaccine
{
    ...
    {
        return "VaccineComponent [diseaseImmunityList=%s, getCdsListItemName()=%s, isLiveVirusVaccine()=%s, isSelectAdjuvantProduct()=%s, getValidMinimumAgeForUse()=%s, getValidMaximumAgeForUse()=%s, getTradeName()=%s, getManufacturerCode()=%s, getLicensedMinimumAgeForUse()=%s, getLicensedMaximumAgeForUse()=%s, isUnspecifiedFormulation()=%s]".formatted(
                diseaseImmunityList, getCdsConceptName(), isLiveVirusVaccine(), isSelectAdjuvantProduct(),
                getValidMinimumAgeForUse(), getValidMaximumAgeForUse(), getTradeName(), getManufacturerCode(),
                getRecommendedMinimumAgeForUse(), getRecommendedMaximumAgeForUse(), isUnspecifiedFormulation());
    }
}
