...
public class SupportedCdsConcepts
{
     ...
     */
    public void addSupportedCdsConcept(final ICEConceptType pICT, final CdsConcept pIC)
    {
     ...
     */
    public boolean existsSupportedConceptsForSpecifiedICEConceptType(final ICEConceptType pICT)
    {
        if (pICT == null)
            return false;

        return !ObjectUtils.isEmpty(this.iceConceptTypeToConceptCdsListItemMap.get(pICT));
    }
     ...
     */
    public Collection<CdsConcept> getOpenCDSICEConceptsAssociatedWithCdsListItem(final LocallyCodedCdsListItem pLCCLI)
    {
        if (pLCCLI == null)
            return List.of();

        final Set<CdsConcept> lSetOfICEConceptsForLC = this.cdsListItemToConceptList.get(pLCCLI);
        if (lSetOfICEConceptsForLC == null)
            return List.of();

        return lSetOfICEConceptsForLC.stream().filter(lIC -> lIC != null && lIC.isOpenCdsSupportedConcept()).toList();
    }

    /**
     * Return list of ICEConcepts for the given LocallyCodedCdsListItem. Returns empty list if none found
     */
    public Collection<CdsConcept> getICEConceptsAssociatedWithCdsListItem(final LocallyCodedCdsListItem pLCCLI)
    {
        if (pLCCLI == null)
            return new ArrayList<>();

        return cdsListItemToConceptList.getOrDefault(pLCCLI, new HashSet<>());
    }
