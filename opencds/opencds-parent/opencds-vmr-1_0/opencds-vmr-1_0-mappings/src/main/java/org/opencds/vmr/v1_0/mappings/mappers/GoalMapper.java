...
public class GoalMapper extends GoalBaseMapper {
	...
	private static Log logger = LogFactory.getLog(GoalMapper.class);
	...
	public static void pullIn(
			org.opencds.vmr.v1_0.schema.Goal 				source, 
			Goal 											target, 
		...
		if ( source.getGoalObserverEventTime() != null ) target.setGoalObserverEventTime(MappingUtility.iVLTS2IVLDateInternal(source.getGoalObserverEventTime()));
		if ( source.getGoalStatus() != null ) target.setGoalStatus(MappingUtility.cD2CDInternal(source.getGoalStatus()));
	 ...
	 * @throws InvalidDataException
	 */
	public static org.opencds.vmr.v1_0.schema.Goal pushOut(
			Goal 											source, 
			org.opencds.vmr.v1_0.schema.Goal		 		target, 
		...
		target = new org.opencds.vmr.v1_0.schema.Goal();
		...
		if ( source.getGoalObserverEventTime() != null ) target.setGoalObserverEventTime(MappingUtility.iVLDateInternal2IVLTS(source.getGoalObserverEventTime()));
		if ( source.getGoalStatus() != null ) target.setGoalStatus(MappingUtility.cDInternal2CD(source.getGoalStatus()));
