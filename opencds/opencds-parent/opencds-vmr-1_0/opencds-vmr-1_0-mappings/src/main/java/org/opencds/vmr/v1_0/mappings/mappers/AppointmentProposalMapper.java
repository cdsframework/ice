...
public class AppointmentProposalMapper extends EncounterBaseMapper {
	...
	private static Log logger = LogFactory.getLog(AppointmentProposalMapper.class);
	...
	public static void pullIn(
			org.opencds.vmr.v1_0.schema.AppointmentProposal	source, 
			AppointmentProposal								target, 
		...
		if ( source.getProposedAppointmentTime() != null ) target.setProposedAppointmentTime(MappingUtility.iVLTS2IVLDateInternal(source.getProposedAppointmentTime()));
	 ...
	 * @throws InvalidDataException
	 */
	public static org.opencds.vmr.v1_0.schema.AppointmentProposal pushOut(
			AppointmentProposal 							source, 
			org.opencds.vmr.v1_0.schema.AppointmentProposal	target, 
		...
		target = new org.opencds.vmr.v1_0.schema.AppointmentProposal();
		...
		if ( source.getProposedAppointmentTime() != null ) target.setProposedAppointmentTime(MappingUtility.iVLDateInternal2IVLTS(source.getProposedAppointmentTime()));
		...
		if (organizedResults.getOutput().getAppointmentProposals() == null) {
			organizedResults.getOutput().setAppointmentProposals(new org.opencds.vmr.v1_0.schema.EvaluatedPerson.ClinicalStatements.AppointmentProposals());
		}
