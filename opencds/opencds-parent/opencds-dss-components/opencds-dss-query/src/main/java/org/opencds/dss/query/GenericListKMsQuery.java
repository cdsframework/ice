...
public class GenericListKMsQuery extends QueryListKMs {
	
	@Override
	public KMList getResponse(String clientLanguage, KMTraitInclusionSpecification kmTraitInclusionSpecification) {
		
		KMList response = new KMList();
		try {
			
			DSSQueryAdapter myDecisionEngineAdapter = DSSQueryAdapter.getInstance();
			response = myDecisionEngineAdapter.listKMs(clientLanguage, kmTraitInclusionSpecification);  
