...
public class GenericGetKMDataRequirementsQuery extends QueryGetKMDataRequirements {
	
	@Override
	public KMDataRequirements getResponse(EntityIdentifier kmId) {
		
		KMDataRequirements response = new KMDataRequirements();
		try {
			
			DSSQueryAdapter myDecisionEngineAdapter = DSSQueryAdapter.getInstance();
			response = myDecisionEngineAdapter.getKMDataRequirements(kmId);  
