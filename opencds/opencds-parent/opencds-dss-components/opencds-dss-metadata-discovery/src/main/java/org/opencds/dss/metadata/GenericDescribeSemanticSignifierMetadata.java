...
public class GenericDescribeSemanticSignifierMetadata extends MetadataDescribeSemanticSignifier {
	
	@Override
	public SemanticSignifier getResponse(EntityIdentifier semanticSignifierId) {
		
		SemanticSignifier response = null;
		try {
			
			DSSMetadataDiscoveryAdapter myDecisionEngineAdapter = DSSMetadataDiscoveryAdapter.getInstance();
			response = myDecisionEngineAdapter.describeSemanticSignifier(semanticSignifierId);  
