...
public class GenericDescribeTraitMetadata extends MetadataDescribeTrait {
	
	@Override
	public Trait getResponse(EntityIdentifier traitId) {
		
		Trait response = new Trait();
		try {
			
			DSSMetadataDiscoveryAdapter myDecisionEngineAdapter = DSSMetadataDiscoveryAdapter.getInstance();
			response = myDecisionEngineAdapter.describeTrait(traitId);  
