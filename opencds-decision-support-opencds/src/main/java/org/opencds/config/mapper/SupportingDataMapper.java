...
public abstract class SupportingDataMapper
{
    public static SupportingData internal(final org.opencds.config.schema.SupportingData external)
    {
        ...
        return SupportingDataImpl.create(external.getIdentifier(), kmId, pkgType, pkgId,
                PluginIdMapper.internal(external.getLoadedBy()), new Date(), external.getUserId());

    }

    public static List<SupportingData> internal(final SupportingDataList external)
    {
        final List<SupportingData> internal = new ArrayList<>();
        for (final org.opencds.config.schema.SupportingData sd : external.getSupportingData())
    ...
    public static SupportingDataList external(final List<SupportingData> supportingDataInternal)
    {
        ...
        for (final SupportingData internalSD : supportingDataInternal)
